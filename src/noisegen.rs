...
static RANDOM_VECTORS_TABLE: [[f64; 4]; 256] = [
...
/// noise, see the comments for the
/// [`gradient_noise3d()`](fn.gradient_noise3d.html) function.
pub fn gradient_coherent_noise3d(x: f64, y: f64, z: f64, seed: i32, quality: NoiseQuality) -> f64 {
    ...
    // interpolation.)
    let n0 = gradient_noise3d(x, y, z, x0, y0, z0, seed);
    let n1  = gradient_noise3d(x, y, z, x1, y0, z0, seed);
    let ix0 = linear_interp(n0, n1, xs);

    let n0 = gradient_noise3d(x, y, z, x0, y1, z0, seed);
    let n1 = gradient_noise3d(x, y, z, x1, y1, z0, seed);
    let ix1 = linear_interp(n0, n1, xs);
    let iy0 = linear_interp(ix0, ix1, ys);

    let n0 = gradient_noise3d(x, y, z, x0, y0, z1, seed);
    let n1 = gradient_noise3d(x, y, z, x1, y0, z1, seed);
    let ix0 = linear_interp(n0, n1, xs);

    let n0 = gradient_noise3d(x, y, z, x0, y1, z1, seed);
    let n1 = gradient_noise3d(x, y, z, x1, y1, z1, seed);
    let ix1 = linear_interp(n0, n1, xs);
...
/// *value*-noise function.  Most noise modules use gradient noise for this
...
/// steps:
///   * It first calculates a random normalized vector based on the nearby
///     integer value passed to this function.
///   * It then calculates a new value by adding this vector to the nearby
///     integer value passed to this function.
///   * It then calculates the dot product of the above-generated value and the
...
pub fn gradient_noise3d(fx: f64, fy: f64, fz: f64, ix: i32, iy: i32, iz: i32, seed: i32) -> f64 {
    ...
    let zv_gradient = RANDOM_VECTORS_TABLE[vec_idx.0 as usize][2];
...
/// input value.
///
///   * `x` - The integer @a x coordinate of the input value.
///   * `y` - The integer @a y coordinate of the input value.
///   * `z` - The integer @a z coordinate of the input value.
...
/// A noise function differs from a random-number generator because it always
/// returns the same output value if the same input value is passed to it.
pub fn i32_value_noise3d(x: i32, y: i32, z: i32, seed: i32) -> i32 {
    // All constants are primes and must remain prime in order for this noise
    // function to work correctly.
    use std::num::Wrapping;
    let n = (
        Wrapping(X_NOISE_GEN) * Wrapping(x)
        + Wrapping(Y_NOISE_GEN) * Wrapping(y)
        + Wrapping(Z_NOISE_GEN) * Wrapping(z)
        + Wrapping(SEED_NOISE_GEN) * Wrapping(seed)) & Wrapping(0x7fffffff);
    let n = (n >> 13) ^ n;
    ((n * (n * n * Wrapping(60493) + Wrapping(19990303)) + Wrapping(1376312589))
...
/// variables of type `i32`.  Before calling a noise function, pass the `x`,
/// `y`, and `z` coordinates to this function to ensure that these coordinates
/// can be cast to an `i32` value.
///
...
/// three-dimensional input value.
///
///   * `x` - The @a x coordinate of the input value.
///   * `y` - The @a y coordinate of the input value.
///   * `z` - The @a z coordinate of the input value.
///   * `seed` - The random number seed.
///   * `quality` - The quality of the coherent-noise.
///
/// The return value ranges from -1.0 to +1.0.
...
/// noise, see the comments for the
/// [`gradient_noise3d()`](fn.gradient_noise3d.html) function.
pub fn value_coherent_noise3d(x: f64, y: f64, z: f64, seed: i32, quality: NoiseQuality) -> f64 {
    // Create a unit-length cube aligned along an integer boundary.  This cube
    // surrounds the input point.
    let x0 = if x > 0.0 { x as i32 } else { (x - 1.0) as i32 };
    let x1 = x0 + 1;
    ...
    // noise values using the S-curve value as the interpolant (trilinear
    // interpolation.)
    let n0 = value_noise3d(x0, y0, z0, seed);
    let n1 = value_noise3d(x1, y0, z0, seed);
    let ix0 = linear_interp(n0, n1, xs);

    let n0 = value_noise3d(x0, y1, z0, seed);
    let n1 = value_noise3d(x1, y1, z0, seed);
    let ix1 = linear_interp(n0, n1, xs);
    let iy0 = linear_interp(ix0, ix1, ys);

    let n0 = value_noise3d(x0, y0, z1, seed);
    let n1 = value_noise3d(x1, y0, z1, seed);
    let ix0 = linear_interp(n0, n1, xs);

    let n0 = value_noise3d(x0, y1, z1, seed);
    let n1 = value_noise3d(x1, y1, z1, seed);
    let ix1 = linear_interp(n0, n1, xs);
    let iy1 = linear_interp(ix0, ix1, ys);
...
/// A noise function differs from a random-number generator because it always
/// returns the same output value if the same input value is passed to it.
pub fn value_noise3d(x: i32, y: i32, z: i32, seed: i32) -> f64 {
